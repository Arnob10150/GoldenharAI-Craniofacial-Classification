...
export default function ScanDetail() {
  ...
  if (!profile || !scan || !child) {
    return <PageTransition><Card><CardContent className="p-6">Scan not found or you do not have access to it.</CardContent></Card></PageTransition>;
  }

  return (
    <PageTransition className="space-y-6">
      <PageHeader title={`Scan detail - ${child.name}`} description={`Full review of the scan completed ${formatDateTime(scan.created_at)}.`} actions={<div className="flex gap-2"><Button variant="outline" onClick={() => downloadPatientReport(scan, child, profile.language_pref)}>Patient PDF</Button><Button onClick={() => downloadClinicalReport(scan, child, profile, referrals[0])}>Clinical PDF</Button></div>} />

      <ScanResultPanel scan={scan} child={child} referral={referrals[0]} onDownloadPatient={() => downloadPatientReport(scan, child, profile.language_pref)} onDownloadClinical={() => downloadClinicalReport(scan, child, profile, referrals[0])} />

      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <Card className="border-border/70 shadow-sm">
          <CardHeader>
            <CardTitle>Clinical notes</CardTitle>
            <CardDescription>Capture doctor interpretation and pathway adjustments.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Care pathway notes</label>
        ...
        <Card className="border-border/70 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Track the full journey of referrals attached to this scan.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {referrals.length ? referrals.map((referral) => (
              <div key={referral.id} className="rounded-3xl border border-border/60 bg-card/60 p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <div className="font-semibold">{referral.specialty}</div>
                    <div className="text-sm text-muted-foreground">To {getProfileDisplayName(referral.to_doctor)} - from {getProfileDisplayName(referral.from_doctor)}</div>
                  </div>
            ...
            )) : <div className="rounded-2xl border border-dashed border-border/70 p-4 text-sm text-muted-foreground">No referrals attached to this scan yet.</div>}
          </CardContent>
        </Card>
      </div>
      ...
      {previousScan ? (
        <Card className="border-border/70 shadow-sm">
          <CardHeader>
            <CardTitle>Compare with previous scan</CardTitle>
            <CardDescription>Side-by-side severity and explainability comparison for longitudinal review.</CardDescription>
          </CardHeader>
            ...
            {[previousScan, scan].map((item, index) => (
              <div key={item.id} className="rounded-3xl border border-border/60 bg-card/60 p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-semibold">{index === 0 ? "Previous scan" : "Current scan"}</div>
